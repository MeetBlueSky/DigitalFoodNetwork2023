			...
			columnDefs: [
				{
					...
					render: function (data, type, full, meta) {
						return '<div class="dropdown dropdown-inline"> ' +
							...
							'		<li class="nav-item"><a class="nav-link" href="#" onclick="staticContentTempDuzenle(' + data + ')"><i class="nav-icon la la-edit"></i><span class="nav-text">Düzenle</span></a></li> ' +
							'		<li class="nav-item"><a class="nav-link" href="#" onclick="staticContentTempSil(' + data + ')"><i class="nav-icon la la-trash"></i><span class="nav-text">Sil</span></a></li> ' +
							'	</ul> ' +
							'</div> ' +
							'</div> ';
					},
...
var secilendeger;
function staticContentTempDuzenle(pid) {
	update = true;
	$('#exampleModalSizeLg').modal('show');
...
function staticContentTempKaydet() {

	$('#exampleModalSizeLg').pleaseWait();
	var staticContentTemp = {
		'Id': 0,
		'Name': $('#p_adi').val(),
		'Desc': $('#p_Ack').val(),
	...
	var dtt;
	if (update) {
		secilendeger.Name = staticContentTemp.Name;
		secilendeger.Desc = staticContentTemp.Desc;

		dtt = { pro: secilendeger };
	} else {
		dtt = { pro: staticContentTemp };
	}
	...
	$.ajax({
		...
		cache: false,
		type: "POST",
		url: '/' + lngg + '/StaticContentTemp/CreatedStaticContentTemp',
...
function staticContentTempSil(pid) {
	...
	swal.fire({
		...
		if (data.isConfirmed) {
			...
			$.ajax({
				...
				cache: false,
				type: "POST",
				url: '/' + lngg + '/StaticContentTemp/DeleteStaticContentTemp',
				success: function (data) {
