...
namespace DFN2023.Business
{
    public class WebsiteService : IWebsiteService
    {
        ...
        // IRepository<CommunicationForm> _fkRepositoryCommunicationForm;
        // IRepository<NewsLetter> _fkRepositoryNewsLetter;
        ...
        public static string getMonthName(int month, int langId)
        {
            string monthName = "";
            ...
            {
                case 1:
                    monthName = langId == 1 ? "Ocak" : "January";
                    break;

                case 2:
                    monthName = langId == 1 ? "Şubat" : "February";
                    break;

                case 3:
                    monthName = langId == 1 ? "Mart" : "March";
                    break;

                case 4:
                    monthName = langId == 1 ? "April" : "Nisan";
                    break;

                case 5:
                    monthName = langId == 1 ? "Mayıs" : "May";
                    break;

                case 6:
                    monthName = langId == 1 ? "Haziran" : "June";
                    break;

                case 7:
                    monthName = langId == 1 ? "Temmuz" : "July";
                    break;

                case 8:
                    monthName = langId == 1 ? "Ağustos" : "August";
                    break;

                case 9:
                    monthName = langId == 1 ? "Eylül" : "September";
                    break;

                case 10:
                    monthName = langId == 1 ? "Ekim" : "October";
                    break;

                case 11:
                    monthName = langId == 1 ? "Kasım" : "November";
                    break;

                case 12:
                    monthName = langId == 1 ? "Aralık" : "December";
                    break;
        ...
        public List<StaticContentPageDTO> getStaticContentByGrupId(int groupId, int lang)
        {

            var result = _mapper.Map<List<StaticContentPageDTO>>(_fkRepositoryStaticContentPage.Entities.Where(p => p.GrupId == groupId && p.LangId == lang && p.Statu == 1).OrderBy(p => p.OrderNo).ToList());


            return result;
        }

        public StaticContentPageDTO getStaticContentByGrupIdLatest(int groupId, int lang)
        {

            var result2 = _mapper.Map<List<StaticContentPageDTO>>(_fkRepositoryStaticContentPage.Entities.Where(p => p.GrupId == groupId && p.LangId == lang && p.Statu == 1).OrderBy(p => p.OrderNo).Take(1).ToList());
            StaticContentPageDTO result = result2[0];

            return result;
        }

        public List<StaticContentPageDTO> getStaticContentByTempId(int tempId, int lang)
        {

            var result = _mapper.Map<List<StaticContentPageDTO>>(_fkRepositoryStaticContentPage.Entities.Where(p => p.TempId == tempId && p.LangId == lang && p.Statu == 1).OrderBy(p => p.OrderNo).ToList());
        ...
        /*
        public List<SlaytDTO> getSlayt()
        {
            return _mapper.Map<List<SlaytDTO>>(_fkRepositorySlayt.Entities.Where(p => p.Status == 1).OrderBy(p => p.RowNum).ToList());
        }*/

        public List<StaticContentPageDTO> getNewsDetail(int newsId, int lang)
        {
            List<StaticContentPage> newsList = new List<StaticContentPage>();
            //var result = _mapper.Map<List<StaticContentPageDTO>>(_fkRepositoryStaticContentPage.Entities.Where(p => p.TempId == tempId && p.LangId == lang && p.Statu == 1).OrderBy(p => p.OrderNo).ToList());
            newsList.Add(_fkRepositoryStaticContentPage.Entities.Where(p => p.Id == newsId && p.LangId == lang && p.Statu == 1).FirstOrDefault());
            newsList.AddRange(_fkRepositoryStaticContentPage.Entities.Where(p => p.Id != newsId && p.GrupId == 6 && p.LangId == lang && p.Statu == 1).OrderBy(p => p.OrderNo).ToList());


            return _mapper.Map<List<StaticContentPageDTO>>(newsList);
        }
